...
model<-mirt.model(s)
mirtmod<-mirt(m,model=model,itemtype = "2PL",TOL=1E-4, optimizer="nlminb",dentype='EHW')
Pars<-coef(mirtmod,simplify=TRUE,IRTPars=FALSE)
Theta <- fscores(mirtmod, full.scores = TRUE, scores.only = TRUE)
write.csv(Pars,"R_EHW_pars.csv")
write.csv(Theta,"R_EHW_theta.csv")
write.csv(mirtmod@OptimInfo$iter,"iter_EHW.csv")
write.csv(mirtmod@time[1],"time_EHW.csv")
